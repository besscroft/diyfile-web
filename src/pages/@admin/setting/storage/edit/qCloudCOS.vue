...
const handleFormData = () => {
  ...
  secretId.value.configValue = updateStorageForm.secretId
    ...
    if (res.code === ResultEnum.SUCCESS) {
      ...
      for (const item of configList) {
        if (item.configKey === 'mount_path') {
          ...
          endpoint.value = item
        } else if (item.configKey === 'secretId') {
          updateStorageForm.secretId = item.configValue
          secretId.value = item
    ...
    <n-grid cols="5" item-responsive responsive="screen">
      <n-grid-item offset="0 m:1 l:1" span="5 m:3 l:3">
        <n-form ref="formRef" :model="updateStorageForm" :rules="rules">
          ...
          <n-form-item label="AccessKey" path="secretId" required>
            <n-input v-model:value="updateStorageForm.secretId" :placeholder="secretId.description" clearable />
          </n-form-item>
          ...
          <n-form-item :label="t('storage.remark')" path="remark">
            <n-input v-model:value="updateStorageForm.remark" type="textarea" placeholder="请输入备注，如果需要访问文件，记得去腾讯云后台配置跨域访问CORS设置" maxlength="160" show-count />
          </n-form-item>
        </n-form>
      </n-grid-item>
    </n-grid>
